    ...
    useEffect(() => {
        getData(id)
            .then(data => {
                ...
                form.setValue('is_free', data.is_free);
                form.setValue('lessons_count', data.lessons_count);
                form.setValue('difficulty', data.difficulty);
                form.setValue('price', data.price);
                form.setValue('category_id', data.category.id);
                form.setValue('language_id', data.language.id);
                ...
                <>
                    <Breadcrumb>
                        <BreadcrumbList>
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbLink asChild>
                                    <Link href={`/courses/${id}`}>{data.title}</Link>
                                </BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbSeparator />
                            <BreadcrumbItem>
                    ...
                    <div className="grid gap-6 md:flex md:gap-12 grid-cols-1 justify-between w-full mt-6">
                        <div className="flex gap-1">
                            <h1 className=" font-medium text-muted-foreground text-nowrap">Edit course #{data.id} - </h1>
                            <span className=" font-medium text-foreground">{data.title}</span>
                        </div>
                        <Select onValueChange={(value) => router.push(`/courses/${value}/edit`)} defaultValue={`${data.id} - ${data.title}`}>
